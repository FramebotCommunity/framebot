...
int _document(){
	...
	fflush(stdout);
	result = send_document_chat(_bot, chat_id, filename, NULL, RAW, ON, 0, NULL);
	if(result){
		printf(BLUE "OK\n" COLOR_RESET);
		message_free(result);
	}
	...
	fflush(stdout);
	result = send_document_chat(_bot, chat_id, filename, "caption", RAW, ON, 0, NULL);
	if(result){
		printf(BLUE "OK\n" COLOR_RESET);
		message_free(result);
	}
	...
	fflush(stdout);
	Message * forward = send_document_chat(_bot, chat_id, filename, "caption", RAW, ON, result->message_id, NULL);
	if(result){
		printf(BLUE "OK\n" COLOR_RESET);
		message_free(result);
